...
for k in range(repeat):
    for i in range (buyers):
        for j in range(goods):
            x_t= np.random.uniform(-0.01, 0.01, time)
            x_t += desired_mean - np.mean(x_t)
            for t in range (time):
                random_error[t,i,j]=x_t[t]
    for i in range(buyers):
       for j in range(goods):
    ...
    for t in range(1,time):
        ...
        for i in range (buyers):
            for j in range(goods): 
                    ...
                    valuesum[i]=0
    #for t in range (time): 
        #print('%f'%value[t][0][0],'%f'%value[t][0][1],'%f'%value[t][0][2],'%f'%value[t][0][3],'%f'%value[t][0][4],'%f'%value[t][0][5],'%f'%value[t][0][6],'%f'%value[t][0][7],'%f'%value[t][0][8],'%f'%value[t][0][9])        
    ...
    for t in range (time):
        for i in range(buyers):
            for j in range (goods):
                ...
                avg_qua[i][j]=avg_qua[i][j]+quantity[t][i][j]
    for i in range(buyers):
        for j in range (goods):
            avg_val[i][j]=(avg_val[i][j]/time)
            avg_qua[i][j]=(avg_qua[i][j]/time)
    ...
    for i in range(buyers):
        for j in range (goods):
            price_opt[j]=price_opt[j]+bidprice_opt[i][j]
    for i in range(buyers):
        for j in range (goods):        
            quantity_opt[i][j]=bidprice_opt[i][j]/price_opt[j]
    ...
    for i in range(buyers):
        for t in range(time):
            for j in range(goods):
                u[t][i]=u[t][i]+avg_val[i][j]*quantity[t][i][j]
                u_opt[t][i]=u_opt[t][i]+avg_val[i][j]*quantity_opt[i][j]

    for t in range(time):
        for i in range(buyers):
            U[i]=U[i]+u[t][i]
            U_opt[i]=U_opt[i]+u_opt[t][i]
            individual_regret[i]=(U_opt[i]-U[i])/U_opt[i]
    ...
    for t in range(time):
        for i in range(buyers):
            for j in range(goods):
                Unsw=Unsw+(bidprice[t][i][j]*avg_val[i][j]-bidprice[t][i][j]*math.log(price[t][j]))
        Q[t]=(OPTnsw-Unsw)
