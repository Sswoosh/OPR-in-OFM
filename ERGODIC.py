...
for k in range(repeat):
    ...
    for i in range (buyers):
        for j in range(goods):       
            ...
            valuesum[i]=0
    t=0
    distribution=np.random.normal(0,0.01,100)
    noise=[]
    for i in range (buyers):
        for j in range (goods):
            random_error[0][i][j]=np.random.normal(0,0.01)
    for t in range (1,time):
        for i in range (buyers):
            for j in range (goods):
                random_error[t][i][j]=0.6*random_error[t-1][i][j]+np.random.normal(0,0.01)
...
# 將DataFrame寫入Excel文件
with pd.ExcelWriter('ergodic.xlsx', engine='xlsxwriter') as writer:
