...
repeat=50
buyers,goods  = 20, 30
sample_path=np.zeros((buyers,goods,time),dtype=int)
...
s = np.random.uniform(size=goods)
s = s / np.sum(s)
for i in range(buyers):
...
for k in range(repeat):
    ...
    for i in range (buyers):
        for j in range(goods):
            ...
            bidprice_exp[0][i][j]=budget[0][i]/(time*goods)
            price[0][j]=price[0][j]+bidprice[0][i][j]
            price_exp[0][j]=price_exp[0][j]+bidprice_exp[0][i][j]
    for i in range (buyers):
        for j in range(goods):       
            quantity[0][i][j]=bidprice[0][i][j]/price[0][j]
            quantity_exp[0][i][j]= bidprice_exp[0][i][j]/price_exp[0][j]
            valuesum[i]=0
    t=0
    initial_distribution=np.random.normal(0,(0.01),size=(buyers,goods,1000))
    plen = 100
    n_periods = int(time//plen)+1
    for i in range(buyers):
        for j in range(goods):
            distributions = np.random.exponential(size=(plen, 1000))
            ...
            for rep in range(1):
                ...
                for period in range(n_periods):
                    curr_period_items = []
                    for v in range(plen):
                        curr_period_items.append(np.random.choice(initial_distribution[i][j], p=distributions[v]))
                    np.random.shuffle(curr_period_items)
                    sample_path.extend(curr_period_items)
                all_sample_paths.append( sample_path)
                for t in range(time):
                    random_error[t][i][j]=all_sample_paths[0][t]
    ...
    for t in range(1,time):
        ...
        for i in range (buyers):
            for j in range(goods): 
                    ...
                    #if bidprice[t][i][j]==0:
                    #    bidprice[t][i][j]=0.0000000000000000000000000000000000000001
        for j in range (goods):
                for i in range (buyers):
    ...
    for t in range(time):
        ...
        #OPTnsw=0
        Unsw=0
        #if  t==2000 or t==4000 or t==6000 or t==8000 or t==9999:
            #print(OPTnsw,Unsw)
...
# 將DataFrame寫入Excel文件
with pd.ExcelWriter('periodic.xlsx', engine='xlsxwriter') as writer:
